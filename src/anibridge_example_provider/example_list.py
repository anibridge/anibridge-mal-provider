...
class ExampleListProvider(ListProvider):
    ...
    def resolve_mappings(
        self, mapping: MappingGraph, *, scope: str | None = None
    ) -> MappingDescriptor | None:
        """Resolve a mapping descriptor for this provider.

        The example implementation picks a descriptor matching the provider
        namespace, optionally preferring an exact scope match when supplied.
        """
        descriptors = mapping.descriptors()
        if not descriptors:
            return None

        matching_namespace = [d for d in descriptors if d.provider == self.NAMESPACE]

        if scope is not None:
            scoped = [d for d in matching_namespace if d.scope == scope]
            if scoped:
                return scoped[0]

        if matching_namespace:
            return matching_namespace[0]

        if scope is not None:
            scoped = [d for d in descriptors if d.scope == scope]
            if scoped:
                return scoped[0]
    ...
    def _encode_entry(self, entry: ListEntry[ExampleListProvider]) -> dict[str, Any]:
        ...
        return {
            ...
            "total_units": entry.media().total_units,
        }
